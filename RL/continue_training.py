...
def main():
    ...
    if args.load_from_checkpoint:
        ...
        start_policy = torch.load("RL/results/default_after_update_1675.pt")
        central_policy.load_state_dict(start_policy)
        central_policy.to("cpu")
        rollout_manager.update_policy(central_policy.state_dict(), policy_id=0)
        central_policy.to(device)
        earlier_policies = deque(maxlen=args.num_policies_to_store)
        files = os.listdir("RL/results")
        files = [file for file in files if file.startswith("default_after")]
        files = sorted(files, key=lambda x: int(x.split("_")[3][:-3]))
        for file in files:
            policy = torch.load("RL/results/"+file, map_location="cpu")
    ...
    rollout_manager.update_annealing_factor(0.325)
    ...
    def run_update():
        ...
        if update_num > args.dense_reward_anneal_start:
            ...
            value = 0.325 + ((update_num - start_update) / (end_update - start_update)) * (1.0 - 0.325)
            rollout_manager.update_annealing_factor(value)

        t_current = time.time()
        print(
            "Updates complete: {}. Total policy steps: {}. Number of games complete: {}. Total elapsed time: {} hours. Entropy loss: {}.".format(
                update_num+1, steps_per_update * (update_num+1), rollout_storage.games_complete,
                            (t_current - start_time) / 3600.0, entropy_loss
            ))
        ...
        if update_num % args.eval_every == 0 and update_num > 0:
            ...
            torch.save(central_policy.state_dict(),
                       "RL/results/" + args.expt_id + "_after_update_" + str(update_num) + "_after_restart.pt")
